...
function MonadicNomad() {
	...
	this.load = function() {
		...
		console.log(type);
		if (type=="csv") jQuery.get("js/"+app.dataFile, function(data){
		...
		else if (type=="json") jQuery.getJSON("js/"+app.dataFile, function(data){
			that.init(data);
		});
		else if (type=="js")  jQuery.getScript("app/"+app.dataFile, function(){
			that.init(data);
		});
		else if (type=="/knowledge")  jQuery.getScript("app/"+app.dataFile, function(){
			that.init(data);
		});
	...
	this.init = function(data) {
		...
		for (var i = 0; i < app.node_types.length; i++) {
			...
			styles += ".sheet."+type+" header	{ background-image: url('img/"+type+".png'); }\n";
		...
		// fill legend
		$("#info h1").append("<a href='"+app.url+"'>"+app.title+"</a>");
		if (app.showType) {
			for (var i = 0; i < app.node_types.length; i++) {
	...
	this.setKeywords = function(){
		$("#keyword-links").find("ul").html("");
		if(this.selected > -1){
			var id = this.selected;
			var keywords = this.nodes.find(function(x){return x.id == id}).keyWord;
			for(i = 0; i < keywords.length; i++){
				var li = $("<li><a href=''>" + keywords[i] + "</a></li>");
				$("#keyword-links ul").append(li);
			}
			$("#keyword-links ul a").on("click",function(){})
		}
	}
